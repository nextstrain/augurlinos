...
def translate_vcf_feature(sequences, ref, feature):
    ...
    for seqk in sequences.keys():
        ...
        geneVarSites = np.where(np.logical_and(varSite >= start, varSite <= end))[0]
        ...
        aaRepLocs = {i//3:safe_translate( "".join([sequences[seqk][key+start] if key+start in sequences[seqk].keys() else refNuc[key] for key in range(i-i%3,i+3-i%3) ]) )
...
def assign_amino_acid_muts_vcf(prots, path):
    ...
    for fname, prot in prots.iteritems():
        ...
        while i < len(positions):
            ...
            pattern = [ refb+str(pi)+sequences[k][pi] if pi in sequences[k].keys() else "" for k,v in sequences.iteritems() ]
            pats.append(pattern)
            i+=1
        ...
        for i in xrange(len(seqNames)):
            ...
            tree_meta[node_name][fname+'_mutations'] = ",".join(ary[np.where(ary != '')])
